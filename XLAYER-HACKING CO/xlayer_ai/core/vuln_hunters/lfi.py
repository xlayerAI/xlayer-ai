...
class LFIHunter(BaseHunter):
    ...
    async def hunt(self, attack_surface: AttackSurface) -> HunterResult:
        ...
        static_lfi = [p.value for p in self.payloads.get_lfi_payloads(os_type=os_type)]
        ...
        for endpoint, param_name in file_params:
            ...
            if not hypotheses:
                ctx = self._build_attack_context(endpoint, param_name, "lfi", attack_surface)

                def lfi_success(r, _ctx):
                    body = r.body
                    for category, patterns in self.FILE_SIGNATURES.items():
                        if any(re.search(p, body) for p in patterns):
                            return True
                    return False

                attempts = await self._adaptive_test(
                    endpoint, param_name, static_lfi, ctx, lfi_success,
                    extra={"server_os": os_type}
                )
                    ...
                    if attempt.success:
                        self._hypotheses.append(self._create_hypothesis(
                            ...
                            indicators=[VulnIndicator(
                                ...
                                detail=f"AI LFI bypass: {attempt.payload[:60]}",
                                confidence_boost=0.3
                            )],
                            suggested_payloads=[attempt.payload],
                            context={"ai_generated": True, "os_type": os_type}
                        ))
        ...
        for payload in traversal_payloads:
            ...
            if abs(response_length - baseline_length) > 100:
                ...
                llm_result = await self._llm_analyze_response(
                    endpoint, parameter, payload * 5, response, "lfi"
                )
                if llm_result and llm_result.get("vulnerable") and llm_result.get("confidence", 0) > 0.7:
                    ...
                    indicators.append(VulnIndicator(
                        ...
                        detail=f"LLM confirmed path traversal with {llm_result.get('confidence', 0):.0%} confidence",
