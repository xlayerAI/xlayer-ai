...
from xlayer_ai.models.report import (
...
class Reporter:
    ...
    def __init__(self, settings: Optional[Settings] = None, llm_engine=None):
    ...
    async def generate(
        ...
        endpoint_entity: Optional[Dict[str, str]] = None,
    ) -> Report:
        """
        ...
        Args:
            ...
            endpoint_entity: Optional URL -> entity map (Admin/User/Invoice) for report context.

        Returns:
            Complete Report object
        """
        self._endpoint_entity = endpoint_entity or {}
        ...
        for vuln in validated_vulns:
            ...
            finding = await self._enhance_finding_with_llm(finding)
            # LLM-generated safe-code snippet for remediation
            snippet = await self._get_remediation_snippet(finding)
            if snippet:
                finding.vulnerability.remediation.insert(0, f"Safe code example: {snippet}")
    ...
    async def _enhance_finding_with_llm(self, finding: Finding) -> Finding:
        """Optionally enhance a finding with LLM-generated content"""
        if not self._llm or not self._llm.is_ready:
            return finding

        try:
            enhanced = await self._llm.enhance_report({
                "title": finding.title,
                "vuln_type": finding.vulnerability.vuln_type.value,
            ...
            if enhanced.get("executive_description"):
                finding.description = enhanced["executive_description"]
            if enhanced.get("business_impact"):
                finding.business_impact = enhanced["business_impact"]
        except Exception as e:
            logger.debug(f"LLM report enhancement failed: {e}")

        return finding

    async def _get_remediation_snippet(self, finding: Finding) -> Optional[str]:
        """Short safe-code snippet for report (parameterized query, encode output, etc.)."""
        if not self._llm or not self._llm.is_ready:
            return None
        try:
            return await self._llm.get_remediation_snippet(
                vuln_type=finding.vulnerability.vuln_type.value,
                parameter=finding.vulnerability.parameter,
                endpoint=finding.vulnerability.endpoint or "",
            )
        except Exception as e:
    ...
    def _get_technical_details(self, vuln: ValidatedVuln) -> str:
        """Get technical details for the finding. Entity context when endpoint_entity provided."""
        details = [
        ...
        entity = getattr(self, "_endpoint_entity", None) or {}
        if isinstance(entity, dict) and vuln.endpoint and entity.get(vuln.endpoint):
            details.append(f"**Entity context:** {entity.get(vuln.endpoint)}")
...
"""
    ...
    async def _save_html(self, report: Report, path: Path):
        ...
        html = html.replace("{{TARGET_URL}}", html_module.escape(report.metadata.target_url))
        ...
        html = html.replace("{{SUMMARY_TEXT}}", html_module.escape(summary_text))
        
        if report.executive_summary and report.executive_summary.immediate_actions:
            actions_html = "".join(f"<li>{html_module.escape(a)}</li>" for a in report.executive_summary.immediate_actions)
        ...
        for finding in report.findings:
            ...
            finding_html = finding_html.replace("{{ENDPOINT}}", html_module.escape(finding.vulnerability.endpoint))
            finding_html = finding_html.replace("{{PARAMETER}}", html_module.escape(finding.vulnerability.parameter))
            finding_html = finding_html.replace("{{DESCRIPTION}}", html_module.escape(finding.description))
            
            poc_curl = finding.vulnerability.poc.curl_command or "N/A"
            finding_html = finding_html.replace("{{POC_CURL}}", html_module.escape(poc_curl))
            
            remediation_items = "".join(
                f"<li>{html_module.escape(r)}</li>" for r in finding.vulnerability.remediation
            )
