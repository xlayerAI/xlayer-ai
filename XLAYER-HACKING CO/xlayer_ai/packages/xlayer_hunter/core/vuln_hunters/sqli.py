...
XLayer AI Compatible - Injection Category:
...
from xlayer_hunter.tools.payload_manager import PayloadCategory, DatabaseType
...
class SQLiHunter(BaseHunter):
    ...
    Comprehensive SQL Injection detection covering:
    - Error-based SQL injection (database errors in response)
    - Boolean-based blind SQL injection (response differences)
    ...
    async def hunt(self, attack_surface: AttackSurface) -> HunterResult:
        ...
        payloads = self.payloads.get_sqli_payloads(
            db_type=db_type,
            include_time_based=True
        )
        payload_values = [p.value for p in payloads]
        
        for endpoint in attack_surface.testable_endpoints:
            for param in endpoint.parameters:
                await self._establish_baseline(endpoint, param.name)
                
                hypotheses = await self._test_sqli(
                    endpoint, param.name, payload_values, db_type
                )
